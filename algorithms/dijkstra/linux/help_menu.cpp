	...
	switch (ans) {
		case '1':	// CLubes e estádios
			try {
				...
				_perror_block ("ERRO AO MOSTRAR AJUDA A CLUBES E ESTÁDIOS!", 1);
			}
			break;
		...
		case '2':	// Matriz de distância
			try {
				...
				_perror_block ("ERRO AO MOSTRAR AJUDA A MATRIZ DE DISTÂNCIA!", 1);
			}
			break;
		...
		case '3':	// Árvore geradora mínima
			try {
				...
				_perror_block ("ERRO AO MOSTRAR AJUDA A ÁRVORE GERADORA MÍNIMA", 1);
			}
			break;
		...
		case '4':	// Caminho mínimo
			try {
				...
				_perror_block ("ERRO AO MOSTRAR AJUDA A CAMINHO MÍNIMO!", 1);
			}
			break;
		...
		case '5':	// Ajuda ao sair do programa
			try {
				...
				_perror_block ("ERRO AO MOSTRAR AJUDA A SAIR DO PROGRAMA", 1);
			}
			break;
